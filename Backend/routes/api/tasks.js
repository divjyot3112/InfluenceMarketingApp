  ...
  Task.create(
    ...
    (err1, result1) => {
      if (err1) {
        ...
        res
          .status(400)
          .json({ success: false, message: "Task could not be added" });
      }
...
router.get("/", (req, res) => {
  ...
  Task.find({ status: taskStatus.CREATED })
    ...
    .catch((err) => {
      ...
      res
        .status(400)
        .json({ success: false, message: "Tasks could not be fetched!" });
    });
});
  ...
  Task.findOne({ _id: ObjectID(req.params.taskId) })
    .then((task) => {
      if (task) {
        if (task.appliedCandidates.length > 0) {
          ...
          res
            .status(400)
            .json({ success: false, message: "No applied Candidates!" });
        }
      } else {
    ...
    .catch((err) => {
      ...
      res
        ...
        .json({
          success: false,
          message: "Applied Candidates could not be fetched ",
        });
    });
});
