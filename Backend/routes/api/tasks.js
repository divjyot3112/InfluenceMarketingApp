...
router.post("/create", (req, res) => {
    console.log("Inside Create Task Post Request");
    Task.create(
        {
    ...
    Task.find({status: taskStatus.CREATED})
        ...
        .then((tasks) => {
            if (tasks && tasks.length > 0) {
                ...
                res.status(404).json({success: false, message: "No created tasks found"});
            }
        })
        .catch((err) => {
            console.log(err);
            ...
            if (task) {
                if (task.postedBy === req.body.email) {
                    ...
                    if (task.status == taskStatus.CREATED) {
                        ...
                        console.log("Cannot select a candidate at this time")
                        res.status(401).json({message: "Cannot select a candidate at this time"});
                    }
                } else {
...
router.get("/search", (req, res) => {
    ...
    if (req.query.status === taskStatus.ALL) {
        console.log("Finding all tasks for status: " + req.query.status);
    ...
    Task.findOne({_id: ObjectID(req.params.taskId)})
        .then((task) => {
            ...
            if (task) {
                ...
                res
                    .status(404)
                    .json({message: "Error deleting! Task does not exist"});
            }
        })
        .catch((err) => {
            ...
            if (task) {
                ...
                if (selectedCandidates && selectedCandidates.length == 0) {
                    ...
                    selectedCandidates.map((candidate, index) => {
                        console.log("Inside candidate map", candidate);

                        InfluencerProfile.findOne({
                            email: candidate,
                        })
                            .then((influencer) => {
                                console.log("Inside influencer profile ", influencer);
                                ...
                                if (influencer) {
                                    ...
                                    res.status(400).json({
                                        message: "Error in finding Candidates Influencer",
                                    });
                                }
                            })
                            .catch((err) => {
                                res.status(400).json({
                                    message: "Error in finding All Selected Influencer profiles",
                                });
                            });
                    });
                }
